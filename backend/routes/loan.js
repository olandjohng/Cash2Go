...
async function createPnNumber(req) {
  try {
    const qCategoryCode = await builder
      .select({ code: "code" })
      .from("loan_categorytbl")
      .where("loan_category_id", req.loan_category_id);
    const qFacilityCode = await builder
      .select({ code: "code" })
      .from("loan_facilitytbl")
      .where("loan_facility_id", req.loan_facility_id);
    const qCustomerPnNumber = await builder
      .select({ id: "pn_number" })
      .from("loan_headertbl")
...
loanRouter.use((req, res, next) => {
  console.log(`LOAN ROUTER HIT: ${req.method} ${req.path}`);
  next();
});
...
loanRouter.get("/voucher/:id", async (req, res) => {
  ...
  try {
    ...
    const loanHeader = await builder
      .select("lh.*", "c.clname", "c.cfname", "c.cmname")
    ...
    const fullname =
      `${lastname[0]}, ${firstName} ${midInitial} ${extName}`.trim();
    ...
    const voucherInfo = {
      ...
      check_details_2: loanHeader.has_second_check
        ...
        : null,
      check_date_2:
        loanHeader.has_second_check && loanHeader.check_date_2
...
loanRouter.get("/renew/:id", async (req, res) => {
  try {
    const renewLoan = await builder("view_loan_renew")
      .select("*")
      .where("loan_header_id", req.params.id)
      .first();
...
loanRouter.post("/renew", async (req, res) => {
  ...
  const totalInterest = loan_details.reduce(
    (acc, cur) => acc + Number(cur.interest),
    0,
  );

  try {
    const { is_rediscount } = await builder("loan_facilitytbl")
      .select("is_rediscount")
      .where("description", loan_facility)
      .first();

    await builder.transaction(async (t) => {
      // update old loan
      await t("loan_headertbl")
        .where({ loan_header_id: req.body.loan_header_id })
        .update({ status_code: LoanStatus.RENEWED }, ["loan_header_id"]);
      // insert new loan
      const term =
        term_type === "months"
          ? loan_details.length
          : dayjs(loan_details[0].dueDate).diff(dayjs(), "day") + 1;
...
loanRouter.post("/recalculate", async (req, res) => {
  ...
  try {
    const isRediscounting = await builder("loan_facilitytbl")
      .select("is_rediscount")
      .first();
    ...
    await builder.transaction(async (t) => {
      await t("loan_headertbl")
        .where({ loan_header_id: req.body.loan_header_id })
        .update({ status_code: LoanStatus.RESTRUCTURED }, ["loan_header_id"]);
...
loanRouter.get("/edit/:id", async (req, res) => {
  try {
    ...
    const deductions = await builder
      ...
      .join(
        "loan_deductiontbl as ld",
        "ldh.loan_deduction_id",
...
loanRouter.put("/:id", async (req, res) => {
  try {
    ...
    // ADD THIS LOG
    console.log("PUT loan debug:", {
      id,
      term_type,
      loan_details_length: loan_details?.length,
      loan_details_sample: loan_details?.[0],
    });
    ...
    if (
      existingLoan.status_code === "Completed" ||
      existingLoan.status_code === "Closed"
    ) {
      return res
        .status(400)
        .json({ message: "Cannot edit completed or closed loans" });
    }

    const totalInterest = loan_details?.length
      ? loan_details.reduce(
          (acc, cur) => acc + Number(cur.interest || cur.monthly_interest),
          0,
        )
      : 0;

    const term = !loan_details?.length
      ? 0
      : term_type === "months"
        ? loan_details.length
        : (() => {
            const days = loan_details
              .map((d) => dayjs(d.dueDate || d.due_date).diff(dayjs(), "day"))
              .filter((d) => isFinite(d)); // ← filter out any bad date values
    ...
    await builder.transaction(async (t) => {
      ...
      await t("loan_headertbl")
        ...
        .update({
          ...
          total_interest: isFinite(totalInterest) ? totalInterest : 0,
          term: isFinite(term) ? term : 0,
        ...
        const loanDetailsMap = loan_details.map((v) => {
          ...
          return {
            ...
            monthly_amortization: Number(
              v.amortization || v.monthly_amortization,
            ),
            monthly_interest: Number(v.interest || v.monthly_interest),
            monthly_principal: Number(v.principal || v.monthly_principal),
            accumulated_penalty: Number(v.accumulated_penalty || 0),
...
loanRouter.delete("/", async (req, res) => {
  ...
  if (!id) return;
  try {
    const del = await builder("loan_headertbl")
      .where("loan_header_id", id)
      .del();
    // console.log(id, del)
    if (del) {
...
loanRouter.post("/", async (req, res) => {
  const { voucher, deduction, loan_details, isCash, term_type, loan_facility } =
    req.body;
  ...
  const term =
    term_type === "months" ? loan_details.length : getMaxDueDate(loan_details);

  // Final safety net — never allow bad values into DB
  const safeTerm = isFinite(term) && !isNaN(term) ? term : 0;
  const safeTotalInterest =
    isFinite(totalInterest) && !isNaN(totalInterest) ? totalInterest : 0;
  ...
  try {
    await builder.transaction(async (t) => {
      ...
      if (!req.body.has_second_check) {
        ...
        id = await builder("loan_headertbl")
          .insert(
            {
              pn_number: pnNumber,
              check_number: req.body.check_number,
      ...
      if (deductionFormat.length > 0) {
        await builder
          .insert(deductionFormat)
          .into("loan_deduction_historytbl")
          .transacting(t);
      }
...
loanRouter.put("/details/:id", async (req, res) => {
  ...
  try {
    const update = await builder("loan_detail")
      .where({
        loan_header_id: id,
...
loanRouter.get("/category", async (req, res) => {
  const banks = await builder
    .select({ id: "loan_category_id", name: "description", code: "code" })
    .from("loan_categorytbl");
...
loanRouter.get("/facility", async (req, res) => {
  const banks = await builder
    .select({ id: "loan_facility_id", name: "description", code: "code" })
    .from("loan_facilitytbl");
  res.status(200).json(banks);
});

loanRouter.get("/collateral", async (req, res) => {
  const col = await builder
    .select({ id: "collateral_id", name: "description" })
    .from("collateraltbl");
  res.status(200).json(col);
});

loanRouter.get("/penalty", async (req, res) => {
  const penalty = await builder
    .select({ id: "penalty_id", penaltyType: "penalty_type" })
    .from("penaltytbl");
...
loanRouter.post("/attachment/:id", (req, res) => {
  attachment(req, res, async (err) => {
    ...
    try {
      const updateAttachment = await builder("loan_headertbl")
        .where("loan_header_id", "=", id)
        .update({
