...
expensesRouter.get("/", async (req, res) => {
  try {
    const expenseHeader = await builder("expenses_header")
      .select("*")
      .orderBy("date", "desc");
...
expensesRouter.get("/banks", async (req, res) => {
  try {
    const banks = await builder("bank_accounttbl")
      .select({ id: "bank_account_id", name: "bank_name" })
      .where("is_owner_bank", true);
...
expensesRouter.get("/suppliers", async (req, res) => {
  try {
    const supplier = await builder("customertbl").select(
      builder.raw(`
    CASE
      WHEN (cfname IS NULL OR cfname = '') AND (cmname IS NULL OR cmname = '')
        THEN clname
      WHEN (cmname IS NULL OR cmname = '')
        THEN CONCAT_WS(', ', clname, cfname)
      ELSE
        CONCAT_WS(', ', clname, CONCAT(cfname, ' ', LEFT(cmname, 1), '.'))
    END as name
  `),
      { id: "customerid" },
    );
...
expensesRouter.get("/edit/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const expenses_details = await builder("expenses_details")
      .select(
        "id",
        { title: "account_title" },
        "expenses_details.account_title_id",
        "credit",
        "debit",
      )
      .innerJoin(
        "account_titletbl",
        "expenses_details.account_title_id",
        "account_titletbl.account_title_id",
      )
      .where("expenses_header_id", Number(id));
    ...
    const [expenses_header] = await builder("expenses_header")
      .select(
        "id",
        {
          borrower: "payee",
    ...
    const formatVoucherDetails = {
      ...
      borrower: {
        name: expenses_header.borrower,
        id: expenses_header.supplier_id,
      },
...
expensesRouter.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const expenses_details = await builder("expenses_details")
      .select("id", { title: "account_title" }, "credit", "debit")
      .innerJoin(
        "account_titletbl",
        "expenses_details.account_title_id",
        "account_titletbl.account_title_id",
...
expensesRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    if (expense) {
      const format = details.map((v) => ({
        ...v,
        expenses_header_id: expense[0],
      }));
      // console.log(format)
...
expensesRouter.put("/:id", async (req, res) => {
  ...
  try {
    await builder.transaction(async (trx) => {
      const updateId = await trx("expenses_header")
        .update(header)
        .where("id", id);

      const del = await trx("expenses_details")
        .where("expenses_header_id", id)
        .del();
