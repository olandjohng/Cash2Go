...
export default function ExpensesVoucher({
  ...
  const totalCredit = voucher.reduce(
    (acc, cur) => acc + Number(cur.credit || 0),
    0,
  );
  const totalDebit = voucher.reduce(
    (acc, cur) => acc + Number(cur.debit || 0),
    0,
  );
    ...
    <>
      <Grid container spacing="10px">
        <Grid item xs={12}>
          <Box display="flex" justifyContent="end">
            <Button color="success" variant="outlined" onClick={handleAddEntry}>
              Add
            </Button>
          </Box>
        </Grid>
      ...
      <Box mt={2} ml={2}>
        <Box display="flex" gap={1}>
          ...
          <Typography fontStyle="italic" fontSize="14px" letterSpacing="1px">
        ...
        <Box display="flex" gap={1}>
          ...
          <Typography fontStyle="italic" fontSize="14px">
            {numberFormat.format(Math.abs(totalCredit - totalDebit))}
          </Typography>
        </Box>
      </Box>
      <Box display="flex" justifyContent="space-between" mt="10px">
        <Button color="success" variant="outlined" onClick={handlePrevious}>
          Back
        </Button>
        <Button color="success" variant="outlined" onClick={handleComplete}>
          Next
        </Button>
      </Box>
    </>
