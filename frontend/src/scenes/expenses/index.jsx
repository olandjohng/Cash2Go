...
export default function ExpensesPage() {
  ...
  const fetchVoucherNumber = async () => {
    try {
      ...
      // Set the formatted voucher number
      setDetails((prev) => ({
        ...prev,
        voucherNumber: sequenceData.formattedValue,
      }));

      console.log("✅ Voucher number set to:", sequenceData.formattedValue);
    } catch (error) {
  ...
  useEffect(() => {
    if (openExpesesForm && !openEditExpensesForm) {
      console.log("🔄 Fetching new voucher number...");
      fetchVoucherNumber();
    }
  }, [openExpesesForm]);
  ...
  const handleStepComplete = (data) => {
    console.log("Step complete, data:", data);
  ...
  const handleSubmitVoucher = async (data) => {
    console.log("📝 Submitting voucher with number:", data.voucherNumber);
  ...
  const handleUpdateVoucher = async (data) => {
    const input = {
      header: {
        ...
        voucher_number: data.voucherNumber, // already uses voucherNumber so this is fine
        remarks: data.remarks,
      },
  ...
  return (
    <Box padding={2} height="100%" display="flex" flexDirection="column">
      <Header
        title="Disbursement"
        onAddButtonClick={() => setOpenExpensesForm(true)}
      />
          ...
          <ExpensesParentForm activeStep={activeStep}>
            ...
            <FormStep label="Voucher">
              <ExpensesVoucher
                ...
                data={{
                  vouchers: details.voucher_details,
