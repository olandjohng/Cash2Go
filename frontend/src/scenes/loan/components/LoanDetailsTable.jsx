...
export default function LoanDetailsTable({ banks, rows, setRows }) {
  ...
  const handleProcessRowUpdate = useCallback(
    (newRow, oldRow) => {
      const updatedRow = {
        ...newRow,
        amortization:
          (Number(newRow.principal) || 0) + (Number(newRow.interest) || 0),
        isNew: false,
      };
  ...
  useEffect(() => {
    setPrincipalTotal(
      rows.reduce((acc, cur) => acc + (Number(cur.principal) || 0), 0)
    );
    setInterestTotal(
      rows.reduce((acc, cur) => acc + (Number(cur.interest) || 0), 0)
    );
    setAmortizationTotal(
      rows.reduce((acc, cur) => acc + (Number(cur.amortization) || 0), 0)
    );
  }, [rows]);
  ...
  const columns = [
    ...
    },
    {
      ...
      valueGetter: (params) => {
        return (
          (Number(params.row.principal) || 0) +
          (Number(params.row.interest) || 0)
        );
      },
    },
    {
            ...
            setTimeout(() => {
              if (currentFieldIndex < editableFields.length - 1) {
                ...
                if (apiRef.current) {
                  apiRef.current.setCellFocus(nextRowId, editableFields[0]);
                  setTimeout(
                    () => startEditingCell(nextRowId, editableFields[0]),
                    50
                  );
                }
              }
