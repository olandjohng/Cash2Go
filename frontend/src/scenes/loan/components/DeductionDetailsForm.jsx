...
export default function DeductionDetailsForm({
  deductions,
  deductionsData,
  setDeductionsData,
}) {
  const [deductionItem, setDeductionItem] = useState(null);
  const { formValue, setFormValue, validationError, setValidationError } =
    ...
    <>
      <Grid container>
        <Grid item xs={10}>
          <Autocomplete
            ...
            onInputChange={(e, v) => {
              console.log(v);
              if (e) setDeductionItem({ id: e.target.id, label: v });
            }}
            ...
            onClick={() => {
              if (deductionItem) {
                ...
                for (const d of deductionsData) {
                  if (d.label === deductionItem.label) {
                    contains = true;
                  }
                }

                if (!contains) {
                  const format = deductionItem.label
                    .toLowerCase()
                    .split(" ")
                    .join("_");
                  const d = [
                    ...deductionsData,
                    { ...deductionItem, name: format, amount: "" },
                  ];
                  setDeductionsData(d);
      ...
      <Box display="flex" marginY={1.5} gap={1.5}>
        <FormControlLabel
          ...
          control={
            <Checkbox
              checked={formValue?.isCash?.value || false} // Add optional chaining
              onChange={(e, checked) =>
                setFormValue((old) => ({
                  ...old,
                  isCash: { ...(old.isCash || {}), value: checked },
                }))
              }
            />
          }
        />
        {formValue?.isCash?.value && ( // Add optional chaining here too
          <TextField
            label="Provisional Receipt"
            size="small"
            value={formValue.isCash.pr_number}
            onChange={(e) =>
              setFormValue((old) => ({
                ...old,
                isCash: { ...old.isCash, pr_number: e.target.value },
              }))
            }
          />
        )}
