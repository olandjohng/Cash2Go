import { Box, IconButton, InputBase, ToggleButton } from "@mui/material";
...
export default function LoanPayment() {
  ...
  const payment_col = [
    ...
    },
    {
      ...
      valueGetter: (params) => {
        const total =
          Number(params.row.payment_principal) +
          Number(params.row.payment_interest) +
          Number(params.row.payment_penalty);
  ...
  const handleSearch = async (value, field) => {
    ...
    try {
      ...
      if (!isDeductionView)
        paymentRowDispatch({ type: "INIT", payments: response.data.data });
      else setDeductionRows(response.data);
  ...
  useEffect(() => {
    if (!isDeductionView) loadPaymentHeaderData();
    else loadDeductionPayment();
  ...
  return (
    ...
    >
      ...
      />
      <Box display="flex" justifyContent="space-between" gap={2} mb={1.2}>
        ...
        <Box display="flex" gap={1}>
          <DatePicker
            ...
            onChange={(val) => setDateView((old) => ({ ...old, from: val }))}
          />
          <DatePicker
            label="to"
            sx={{ width: 150 }}
            value={dateView.to}
            slotProps={{ textField: { size: "small" } }}
            onChange={(val) => setDateView((old) => ({ ...old, to: val }))}
          />
        </Box>
      </Box>
      {/*TODO Filter date UI */}
      ...
      <Box flex={1} position="relative">
        <Box sx={{ position: "absolute", inset: 0 }}>
          {isDeductionView ? (
            <DeductionDataGrid rows={deductionRows} />
          ) : (
            <PaymentDataGrid
              columns={payment_col}
              rows={paymentRows}
      ...
      >
        <PaymentForm
          popup={setOpenPopup}
          paymentDispacher={paymentRowDispatch}
        />
      </Popups>
    </Box>
  );
